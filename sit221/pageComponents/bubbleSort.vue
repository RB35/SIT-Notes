<template>
  <div ref="containerRef" style="width: 100%; height: 100%; outline: 4px black solid;">
    <v-stage :config="{
      ...
      <!--UI layer-->
      <v-layer>
        <v-group @click="bubbleSort" :config="{ x: 235, y: 250 }" @mouseover="handleMouseOver($event, 'pointer')"
        ...
        <v-group :config="{ x: 10, y: 330 }">
          <v-rect :config="{ x: 0, y: 0, width: 25, height: 25, fill: 'red' }" />
          <v-text :config="{ x: 30, y: 5, fontSize: 15, fontStyle: 'bold', text: '= Currently Comparing' }" />
        ...
        <v-text
          :config="{ x: 10, y: 5, text: 'Bubble Sort', fontStyle: 'bold', fontFamily: 'monospace', fontSize: 45, textDecoration: 'underline' }" />
        <v-text
          :config="{ x: 360, y: 10, width: 250, text: 'Note: Double click an element\'s value to change it!', fontStyle: 'bold', fontFamily: 'monospace', fontSize: 13 }" />
      </v-layer>

      <!-- Elements layer -->
      <v-layer ref="arrayLayerRef">
        <v-arrow :config="arrowConfig" ref="arrowRef" />
        <v-group
          :config="{ x: (sceneWidth - (data.length * config.element.width + ((data.length - 1) * config.element.xSpacing))) / 2 }">
          <v-group @mouseover="handleMouseOver($event, 'pointer')" @mouseout="handleMouseOut"
            ...
            <v-text @dblclick="handleTextDblClick" @dbltap="handleTextDblClick" :config="{
              x: 0,
...
function arrowPointAt(index) {
  ...
  let x = element.x() + (config.element.width / 2);
  let y = element.y() - arrowConfig.pointerLength - arrowConfig.points[3];
  arrow.x(x);
  arrow.y(y);
  arrow.show();
}
...
async function bubbleSort() {
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = 0; j < len - i - 1; j++) {
      ...
      setElementColour(j, "red");
      setElementColour(j + 1, "red");
      if (getElementValue(j) > getElementValue(j + 1)) {
        await swap(j, j + 1);
        swapped = true;
      }
      await stepDelay(startTime);
      setElementColour(j, "grey");
      setElementColour(j + 1, "grey");
    }
    setElementColour(len - i - 1, "blue");
    if (!swapped) {
      setAllElementColour("blue");
      break;
    }
  }
  isSorting = false;
