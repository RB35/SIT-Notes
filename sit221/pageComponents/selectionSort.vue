<template>
  <div ref="containerRef" style="width: 100%; height: 100%; outline: 4px black solid;">
    <v-stage :config="{
      ...
      <!--UI layer-->
      <v-layer>
        <v-group @click="selectionSort" :config="{ x: 235, y: 250 }" @mouseover="handleMouseOver($event, 'pointer')"
        ...
        <v-text
          :config="{ x: 10, y: 5, text: 'Selection Sort', fontStyle: 'bold', fontFamily: 'monospace', fontSize: 45, textDecoration: 'underline' }" />
        <v-text
...
import { Stage as VStage, Layer as VLayer, Rect as VRect, Group as VGroup, Text as VText, RegularPolygon as vRegularPolygon, Arrow as VArrow } from 'vue-konva';
...
async function selectionSort() {
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = i + 1; j < len; j++) {
      ...
      if (getElementValue(j) < getElementValue(minIndex)) {
        minIndex = j;
        arrowPointAt(j);
      }
      await delay(1000);
    ...
    arrowHide();
    setElementColour(i, "grey");
    if (minIndex != i) {
      await swap(i, minIndex);
    }
    setElementColour(i, "blue");
    await delay(1000)
  }
  arrowHide();
  isSorting = false;
